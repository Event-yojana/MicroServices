...
namespace EventYojana.Infrastructure.Core.Attributes
{
    ...
    public class FromEncryptedRouteAttribute : Attribute, IModelBinder
    {
        public async Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            if(values.Length > 0)
            {
                ...
                {
                    bindingContext.Result = ModelBindingResult.Success(
                        bindingContext.ModelType == typeof(string) ? value : Convert.ChangeType(value, bindingContext.ModelType));
                }
            }
